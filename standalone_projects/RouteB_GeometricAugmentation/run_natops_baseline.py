...
from core.pia_operator_value_probe import (  # noqa: E402
...
from core.scp_prototype_memory import (  # noqa: E402
    SCPPrototypeMemoryConfig,
    build_scp_prototype_memory,
)
from core.trajectory_feedback_pool_windows import (  # noqa: E402
    build_window_feedback_reference_stats,
)
from core.trajectory_representation import (  # noqa: E402
...
def _mix_state_with_augmented_zseqs(
    state: TrajectoryRepresentationState,
    *,
    aug_train_z_seq_list: Sequence[np.ndarray],
) -> TrajectoryRepresentationState:
    # Concatenate original training and augmented training data
    mixed_z_seq_list = list(state.train.z_seq_list) + [np.asarray(v, dtype=np.float32) for v in aug_train_z_seq_list]
    mixed_y = np.concatenate([state.train.y, state.train.y], axis=0)
    mixed_X_static = np.concatenate([state.train.X_static, state.train.X_static], axis=0)
    
    # Ensure unique identifiers for augmented samples and update metadata
    orig_tids = [str(tid) for tid in state.train.tids]
    aug_tids = [f"{tid}_aug" for tid in state.train.tids]
    mixed_tids = np.array(orig_tids + aug_tids, dtype=object)
    
    # Duplicate and update trial_dicts
    aug_trial_dicts = []
    for tr in state.train.trial_dicts:
        new_tr = dict(tr)
        new_tr["trial_id_str"] = f"{tr['trial_id_str']}_aug"
        aug_trial_dicts.append(new_tr)
    mixed_trial_dicts = list(state.train.trial_dicts) + aug_trial_dicts

    # Duplicate and update window_meta_list
    aug_window_meta_list = []
    for seq_meta in state.train.window_meta_list:
        new_seq_meta = []
        for win_meta in seq_meta:
            new_win_meta = dict(win_meta)
            new_win_meta["trial_id"] = f"{win_meta['trial_id']}_aug"
            new_seq_meta.append(new_win_meta)
        aug_window_meta_list.append(new_seq_meta)
    mixed_window_meta_list = list(state.train.window_meta_list) + aug_window_meta_list

    mixed_log_matrix_seq_list = list(state.train.log_matrix_seq_list) + list(state.train.log_matrix_seq_list)

    train_split = replace(
        state.train, 
        z_seq_list=mixed_z_seq_list,
        y=mixed_y,
        tids=mixed_tids,
        X_static=mixed_X_static,
        trial_dicts=mixed_trial_dicts,
        log_matrix_seq_list=mixed_log_matrix_seq_list,
        window_meta_list=mixed_window_meta_list,
    )
    # Val and Test remain strictly original for evaluation
    return replace(state, train=train_split)
...
def _evaluate_terminal(
    ...
    *,
    ...
    if terminal == "dynamic_minirocket":
        from core.trajectory_minirocket_evaluator import (  # noqa: E402
    ...
    if terminal == "dynamic_gru":
        from core.trajectory_classifier import TrajectoryModelConfig  # noqa: E402
        from core.trajectory_evaluator import (  # noqa: E402
...
def main() -> None:
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            for r_dim in r_dimensions:
                ...
                # Arm: Mixed Training (1+1 Original + Augmented)
                mixed_state = _mix_state_with_augmented_zseqs(state, aug_train_z_seq_list=b_state.train.z_seq_list)
                m_train_metrics, m_val_metrics, m_test_metrics, m_meta = _evaluate_terminal(mixed_state, seed=int(seed), args=args)[1:]
                ...
                arm_results[f"b3_r{int(r_dim)}_mixed_training"] = {
                    "train_res": b_train, # Using b_train as a proxy for summary
                    "val_res": b_val,
                    "test_res": b_test,
                    "summary": b_summary,
                    "train_metrics": m_train_metrics,
                    "val_metrics": m_val_metrics,
                    "test_metrics": m_test_metrics,
                    "meta": m_meta,
                    "after_diag": after_b_diag,
                    "template_count": int(r_dim),
                }
    ...
    for dataset in datasets:
        ...
        for r_dim in r_dimensions:
            ...
            lines.append(f"- `b3_r{int(r_dim)}_mixed_training`: {_format_mean_std(ds[f'b3_r{int(r_dim)}_mixed_training_test_macro_f1'].tolist())}")
