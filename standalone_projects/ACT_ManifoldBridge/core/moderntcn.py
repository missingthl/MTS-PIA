...
class RevIN(nn.Module):
    ...
    def _denormalize(self, x):
        ...
        if self.subtract_last:
            ...
            x = x + self.mean
        return x#__all__ = ['Transpose', 'get_activation_fn', 'moving_avg', 'series_decomp', 'PositionalEncoding', 'SinCosPosEncoding', 'Coord2dPosEncoding', 'Coord1dPosEncoding', 'positional_encoding']
...
class ModernTCN(nn.Module):
    def __init__(self,task_name,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        if self.task_name == 'classification':
            ...
            # Use Global Average Pooling to be robust to sequence length
            self.head_class = nn.Linear(self.n_vars * d_model, self.class_num)
        else:
            # Original logic for forecasting tasks (if needed)
    ...
    def classification(self,x):
        ...
        # Global Average Pooling across sequence dimension N
        # x shape: [B, M, D, N]
        x = torch.mean(x, dim=-1) # [B, M, D]
        x = x.reshape(x.shape[0], -1) # [B, M*D]
        x = self.head_class(x)
        return x
