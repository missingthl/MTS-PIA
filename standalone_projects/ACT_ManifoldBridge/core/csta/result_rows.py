...
from core.csta.result_schema import (
    AG_RESULT_FIELDS,
...
def _aug_train_ratio(pipeline_out: Dict[str, object], y_train: np.ndarray) -> float:
    return float(pipeline_out.get("aug_total_count", 0)) / max(float(len(y_train)), 1.0)


def _copy_existing_fields(summary: Dict[str, object], pipeline_out: Dict[str, object], fields: List[str]) -> None:
    for key in fields:
        if key in pipeline_out:
            summary[key] = pipeline_out[key]


def _set_core_concat_operator_metadata(
    ...
    summary.update(
        {
            ...
            "aug_train_ratio": _aug_train_ratio(pipeline_out, y_train),
...
def _merge_generation_engine_result_fields(
    summary: Dict[str, object],
    *,
    args,
    pipeline_out: Dict[str, object],
    ...
    static_specs = [
        (
            {"ag_target_direct", "ag_pia_single", "ag_pia_multihead5"},
            "ag_pia_operator",
            "covariance_state_operator",
            AG_RESULT_FIELDS,
        ),
        (
            {"cs_flow_target_direct", "cs_flow_single_step"},
            "cs_flow_operator",
            "covariance_state_flow",
            CS_FLOW_RESULT_FIELDS,
        ),
        (
            {"latent_residual_direct", "latent_residual_flow"},
            "latent_residual_flow_operator",
            "covariance_state_latent_residual",
            LATENT_RESIDUAL_RESULT_FIELDS,
        ),
        (
            {"task_guided_residual_direct", "task_guided_latent_residual_flow"},
            "task_guided_latent_residual_operator",
            "covariance_state_task_guided_latent_residual",
            LATENT_RESIDUAL_RESULT_FIELDS + TASK_GUIDED_LATENT_RESULT_FIELDS,
        ),
        (
            {"lc_residual_direct", "lc_latent_residual_flow"},
            "label_consistent_latent_residual_operator",
            "covariance_state_label_consistent_latent_residual",
            LATENT_RESIDUAL_RESULT_FIELDS + LC_LATENT_RESULT_FIELDS,
        ),
        (
            {"gi_spg_pia_zhead"},
            "generalized_inverse_spg_operator",
            "covariance_state_generalized_inverse_spg",
            GI_SPG_RESULT_FIELDS,
        ),
    ]
    for methods, direction_source, source_space, fields in static_specs:
        if algo in methods:
            _set_core_concat_operator_metadata(
                summary,
                args=args,
            ...
            _copy_existing_fields(summary, pipeline_out, fields)
    ...
    if algo in {
        ...
        is_ecl = algo.startswith("ecl_spg_pia_")
        is_rn = algo.startswith("rn_ecl_spg_pia_")
        _set_core_concat_operator_metadata(
            summary,
            args=args,
            pipeline_out=pipeline_out,
        ...
        _copy_existing_fields(summary, pipeline_out, SPG_RESULT_FIELDS)

    if algo in {"spg_cfm_one_step", "spg_cfm_k3", "spg_cfm_film_one_step", "spg_cfm_align_one_step"}:
        _set_core_concat_operator_metadata(
            summary,
            args=args,
            pipeline_out=pipeline_out,
            ...
            direction_source=(
                ...
                if algo == "spg_cfm_film_one_step"
                else "spg_conditioned_cfm_align_operator"
                if algo == "spg_cfm_align_one_step"
                else "spg_conditioned_cfm_operator"
            ),
            source_space="covariance_state_spg_conditioned_cfm",
        )
        _copy_existing_fields(summary, pipeline_out, SPG_CFM_RESULT_FIELDS)


def _merge_zpia_bank_metadata(summary: Dict[str, object], *, args, direction_meta: Dict[str, object]) -> None:
    if direction_meta.get("bank_source") != "zpia_telm2" and args.algo != "zpia":
        return
    summary.update(
        {
...
def _merge_zpia_template_pool_metadata(
    summary: Dict[str, object],
    *,
    args,
    pipeline_out: Dict[str, object],
    ...
    if args.algo not in {"zpia_top1_pool", "zpia_multidir_pool", "rc4_multiz_fused"}:
        return
    summary.update(
        {
            "utilization_mode": "core_concat",
...
def build_success_result_row(
    *,
    ...
    cov_state_compute_sec = float(pipeline_out.get("cov_state_compute_sec", 0.0))
    bridge_realization_sec = float(pipeline_out.get("bridge_realization_sec", 0.0))
    summary = {
        "dataset": dataset_name,
        "seed": seed,
        "status": "success",
        ...
        "downstream_train_sec": float(res_act.get("downstream_train_sec", 0.0)),
        "peak_gpu_mem_mb": float(res_act.get("peak_gpu_mem_mb", 0.0)),
        "cov_state_compute_sec": cov_state_compute_sec,
        "bridge_realization_sec": bridge_realization_sec,
        "aug_cost_sec": cov_state_compute_sec + bridge_realization_sec,
    ...
    _merge_generation_engine_result_fields(summary, args=args, pipeline_out=pipeline_out, y_train=y_train)
    _merge_zpia_bank_metadata(summary, args=args, direction_meta=direction_meta)
    _merge_zpia_template_pool_metadata(
        summary,
        args=args,
        pipeline_out=pipeline_out,
        y_train=y_train,
