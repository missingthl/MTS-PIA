...
def build_acl_candidate_pool(
    ...
    tid_train: np.ndarray,
    *,
    class_basis_bank: Dict[int, Dict[str, np.ndarray]],
    ...
    """
    Generate a per-anchor candidate pool by sampling within class-conditioned
    top-k tangent subspaces.
    """
    X_z = np.asarray(X_train, dtype=np.float64)
    y_arr = np.asarray(y_train).astype(int).ravel()
    tid_arr = np.asarray(tid_train)
    margins = estimate_local_manifold_margins(X_z, y_arr)

    candidates: List[Dict[str, object]] = []
    safe_ratios = []
    margin_values = []

    for anchor_idx in range(len(X_z)):
        cls = int(y_arr[anchor_idx])
        basis = class_basis_bank.get(cls, {})
        axes = np.asarray(basis.get("axes", np.empty((0, X_z.shape[1]))), dtype=np.float64)
        eigvals = np.asarray(basis.get("eigvals", np.empty((0,))), dtype=np.float64).ravel()
        if axes.size == 0 or eigvals.size == 0:
            continue

        spectral_scale = np.sqrt(np.clip(eigvals, a_min=0.0, a_max=None))
        if np.max(spectral_scale) <= 1e-12:
            spectral_scale = np.ones_like(spectral_scale, dtype=np.float64)
        else:
            spectral_scale = spectral_scale / (np.max(spectral_scale) + 1e-12)

        rs = np.random.RandomState(int(seed + anchor_idx * 1009 + _stable_tid_hash(tid_arr[anchor_idx])))
        d_min = float(margins[anchor_idx])
        margin_values.append(d_min)

        for cand_idx in range(max(0, int(candidates_per_anchor))):
            coeff_std = float(gamma_scale) * spectral_scale
            coeff = rs.normal(loc=0.0, scale=coeff_std, size=axes.shape[0])
            delta_z = axes.T @ coeff

            raw_norm = float(np.linalg.norm(delta_z))
            if eta_safe is not None:
                safe_gamma, safe_ratio = apply_safe_step_constraint(1.0, raw_norm, d_min, eta=eta_safe)
                if safe_gamma < 1.0:
                    delta_z = delta_z * safe_gamma
            else:
                safe_ratio = 1.0
            safe_ratios.append(float(safe_ratio))

            safe_norm = float(np.linalg.norm(delta_z))
            z_cand = X_z[anchor_idx] + delta_z

            candidates.append({
                "anchor_index": int(anchor_idx),
                "candidate_local_index": int(cand_idx),
                "tid": tid_arr[anchor_idx],
                "y": cls,
                "z_src": X_z[anchor_idx].astype(np.float32),
                "z_cand": z_cand.astype(np.float32),
                "delta_norm_raw": raw_norm,
                "delta_norm_safe": safe_norm,
                "safe_radius_ratio": float(safe_ratio),
                "manifold_margin": d_min,
                "basis_rank_count": int(axes.shape[0]),
            })
