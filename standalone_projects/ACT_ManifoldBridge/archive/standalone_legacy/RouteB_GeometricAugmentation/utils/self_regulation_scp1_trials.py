...
def _resolve_selfreg_root(path: str | Path) -> Path:
    p = Path(path).expanduser().resolve()
    candidates = (
        p,
        p / "raw_extracted",
        p / "SelfRegulationSCP1",
    )
    for c in candidates:
        if (c / "SelfRegulationSCP1_TRAIN.ts").is_file() and (c / "SelfRegulationSCP1_TEST.ts").is_file():
            return c
    raise FileNotFoundError(
        "SelfRegulationSCP1 root not found. Expected SelfRegulationSCP1_TRAIN.ts and "
        f"SelfRegulationSCP1_TEST.ts. Got: {p}"
    )
...
def _load_ts_file(path: Path) -> Tuple[np.ndarray, List[str], List[str]]:
    ...
    return np.stack(x_list, axis=0).astype(np.float32, copy=False), y_list, _parse_class_order(header)


class SelfRegulationSCP1TrialDataset:
    """Expose SelfRegulationSCP1 UEA-format multivariate time series in common trial dict format."""
    ...
    def __init__(
        ...
        *,
        ...
        self.root = _resolve_selfreg_root(root)
        ...
        train_x, train_y_raw, train_order = _load_ts_file(self.root / "SelfRegulationSCP1_TRAIN.ts")
        test_x, test_y_raw, test_order = _load_ts_file(self.root / "SelfRegulationSCP1_TEST.ts")
    ...
    def __iter__(self) -> Iterator[Dict]:
        for split in self.include_splits:
            ...
            for i in range(x.shape[0]):
                yield {
                    "trial_id_str": f"selfregulationscp1_{split}_n{i:05d}",
                    "x_trial": x[i],
                    "label": int(y[i]),
                    "sfreq": self.sfreq,
                    "subject": -1,
