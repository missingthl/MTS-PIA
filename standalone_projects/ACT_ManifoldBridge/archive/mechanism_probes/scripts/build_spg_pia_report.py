...
def build_report(out_root: Path) -> None:
    ...
    summary.to_csv(out_root / "spg_pia_summary.csv", index=False)

    report = ["# SPG-PIA z-head Report", "", "`spg_pia_zhead_deterministic` is diagnostic only."]
    table_rows = []
    ...
    diag_rows = []
    for r in summary.to_dict("records"):
        if str(r["method"]).startswith("spg_pia_"):
            diag_rows.append([
                r["method"],
                _fmt(r.get("spg_zhead_train_acc")),
                _fmt(r.get("spg_projection_energy")),
                _fmt(r.get("spg_projected_grad_norm_mean")),
                _fmt(r.get("spg_direction_pairwise_cosine_mean")),
                _fmt(r.get("spg_effective_aug_multiplier")),
                _fmt(r.get("spg_support_rank")),
                _fmt(r.get("bridge_success_rate")),
                _fmt(r.get("safe_clip_rate")),
            ])
    report.append("## SPG Diagnostics")
    report.append(_table(["Method", "z-head acc", "Proj Energy", "Proj Norm", "Dir Cos", "Eff Mult", "Support Rank", "Bridge", "Safe Clip"], diag_rows))
    (out_root / "spg_pia_report.md").write_text("\n".join(report), encoding="utf-8")
