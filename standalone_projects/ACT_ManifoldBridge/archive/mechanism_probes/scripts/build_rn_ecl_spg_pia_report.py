...
def build_report(out_root: Path) -> None:
    ...
    summary.to_csv(out_root / "rn_ecl_spg_pia_summary.csv", index=False)

    report = ["# RN-ECL-SPG-PIA Report", "", "`rn_ecl_spg_pia_zhead_deterministic` is diagnostic only. SPG reference cosine is 0.991163; ECL reference cosine is 0.578470."]
    table_rows = []
    ...
    diag_rows = []
    for r in summary.to_dict("records"):
        if str(r["method"]).startswith("spg_pia_") or str(r["method"]).startswith("ecl_spg_pia_") or str(r["method"]).startswith("rn_ecl_spg_pia_"):
            diag_rows.append([
                r["method"],
                _fmt(r.get("spg_zhead_train_acc")),
                _fmt(_first_finite(r.get("rn_ecl_projection_energy_mean"), r.get("ecl_projection_energy_mean"), r.get("spg_projection_energy"))),
                _fmt(_first_finite(r.get("rn_ecl_alpha_mean"), r.get("ecl_alpha_mean"))),
                _fmt(_first_finite(r.get("rn_ecl_alignment_to_projected_gradient_mean"), r.get("ecl_alignment_to_projected_gradient_mean"))),
                _fmt(_first_finite(r.get("rn_ecl_direction_pairwise_cosine_mean"), r.get("ecl_direction_pairwise_cosine_mean"), r.get("spg_direction_pairwise_cosine_mean"))),
                _fmt(_first_finite(r.get("rn_ecl_effective_aug_multiplier"), r.get("ecl_effective_aug_multiplier"), r.get("spg_effective_aug_multiplier"))),
                _fmt(_first_finite(r.get("rn_ecl_support_rank"), r.get("ecl_support_rank"), r.get("spg_support_rank"))),
    ...
    (out_root / "rn_ecl_spg_pia_report.md").write_text("\n".join(report), encoding="utf-8")
