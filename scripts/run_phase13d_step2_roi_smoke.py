...
CONFIG = {
    ...
    "band_norm_mode": "per_band_global_z", # Step 2
    "use_band_gate": False, # Manifold Only (No Gate Module)
    "mvp1_guided_cov": False, # No DCNet Guidance
    "use_roi_pooling": True, # ROI Pooling On (Step 2)
    "mvp1_attn_power": 0.0,
    "epochs": 10,
    "batch_size": 8,
    "spd_eps": 0.001,
    "output_root": "promoted_results/phase13d/step2"
}
...
def run_worker(seed, tag):
    ...
    print(f"[Worker] Starting Seed={seed} Tag={tag} Mode=ROI Pooling")
    print(f"Output: {out_dir}")
    
    # Checkpoint Dir
    rel_path_dir = f"phase13d/step2/{args.dataset}/seed{seed}/{tag}/manifold"
