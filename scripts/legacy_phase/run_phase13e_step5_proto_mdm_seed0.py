...
from runners.prototype_mdm_runner import PrototypeMDMRunner
...
CONFIG = {
    ...
    # Model (Variables Minimal as requested)
    "matrix_mode": "corr",
    ...
    "gate": False,
    "spd_eps": 1e-3,
    "epochs": 1, # Dummy, not used by MDM
    "batch_size": 32, # Faster inference
    "lr": 1e-4, # Dummy
    
    # Base Path
    "rel_path_base_fmt": "phase13e/step5/seed1/seed{}/proto_mdm_logeuclid/manifold"
}
...
def run_worker(seed):
    print(f"\n=== Starting Phase 13E Step 5 Proto MDM (Seed {seed}) ===")
    set_seed(seed)
    ...
    # 3. Run
    runner = PrototypeMDMRunner(args, num_classes=3)
