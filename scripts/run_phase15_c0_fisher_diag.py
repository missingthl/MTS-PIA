#!/usr/bin/env python
"""Phase C0: offline Fisher-PIA direction diagnosis on current Step1B bank."""
...
from scripts.fisher_pia_utils import (  # noqa: E402
    FisherPIAConfig,
    compute_fisher_pia_scores,
    compute_fisher_pia_terms,
    compute_score_correlations,
    summarize_score_signal,
)
from scripts.local_knn_gate import LocalKNNGateConfig, ReadOnlyLocalKNNGate  # noqa: E402
from scripts.run_phase15_k1_knn_gate import (  # noqa: E402
    _apply_gate12_with_diag,
    _merge_gate3_into_dir_profile,
)
from scripts.run_phase15_step1a_maxplane import _apply_window_cap, _fit_gate1_from_train, _make_trial_split  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--out-root", type=str, default="out/phase15_c0_fisher_diag")
    ...
    parser.add_argument("--fisher-alpha", type=float, default=1.0)
    parser.add_argument("--fisher-beta", type=float, default=1.0)
    parser.add_argument("--fisher-gamma", type=float, default=1.0)
    parser.add_argument("--fisher-eta", type=float, default=1.0)
    parser.add_argument("--fisher-rho", type=float, default=1e-6)
    ...
    dataset_out = os.path.join(args.out_root, args.dataset)
    ...
    fisher_cfg = FisherPIAConfig(
        ...
        alpha=float(args.fisher_alpha),
        beta=float(args.fisher_beta),
        gamma=float(args.fisher_gamma),
        eta=float(args.fisher_eta),
        rho=float(args.fisher_rho),
    )

    for seed in seeds:
        print(f"[C0][{args.dataset}][seed={seed}] start")
        ...
        print(f"[C0][{args.dataset}][seed={seed}] train_windows={len(y_train)} dim={X_train.shape[1]}")
        ...
        if bool(args.enable_gate3):
            ...
            X_anchor, y_anchor, tid_anchor = X_train, y_train, tid_train
            anchor_counts = {}
        ...
        class_df, global_df = compute_fisher_pia_scores(direction_bank, class_terms, cfg=fisher_cfg)
        ...
        corr_df = compute_score_correlations(global_df)
        signal_meta = summarize_score_signal(global_df)

        setting_tag = f"kdir{int(args.kdir)}_s{int(args.subset_size)}"
        if bool(args.enable_gate3):
            tau_tag = str(float(args.gate3_tau_purity)).replace(".", "p")
            setting_tag += f"__g3k{int(args.gate3_k)}_tau{tau_tag}"
        run_dir = os.path.join(dataset_out, setting_tag, f"seed{seed}")
        ensure_dir(run_dir)
        ...
        metrics = {
            ...
            "transition_model": "none_offline_score_only",
            "center_update_mode": "none",
            "loss_mode": "offline_score_diagnosis",
            "n_dirs": int(direction_bank.shape[0]),
            "eval_metrics": ["fisher_pia_score", "accept_rate", "flip_rate", "margin_drop_median", "intrusion"],
            **signal_meta,
        }
        run_meta = {
            "dataset": args.dataset,
            ...
            "gate3_anchor_space": "clean_train_only" if bool(args.enable_gate3) else None,
            ...
            "gate3_diag": gate3_diag if gate3_diag is not None else {},
            "fisher_pia_enabled": True,
            "fisher_pia_phase": "C0",
            "fisher_pia_mode": "score_only",
            "fisher_pia_alpha": float(args.fisher_alpha),
            "fisher_pia_beta": float(args.fisher_beta),
            "fisher_pia_gamma": float(args.fisher_gamma),
            "fisher_pia_eta": float(args.fisher_eta),
            "fisher_pia_rho": float(args.fisher_rho),
            "fisher_pia_k": int(args.kdir),
            "fisher_pia_score_normalization": "class_weighted_mean",
            "fisher_pia_temperature": None,
            "fisher_terms_meta": terms_meta,
            "score_signal": signal_meta,
            "mech": mech,
        }
        _write_condition(run_dir, metrics, run_meta)
        combined_df.to_csv(os.path.join(run_dir, "fisher_pia_direction_table.csv"), index=False)
        global_df.to_csv(os.path.join(run_dir, "fisher_pia_direction_table_global.csv"), index=False)
        corr_df.to_csv(os.path.join(run_dir, "fisher_pia_score_correlation.csv"), index=False)
        ...
        row = {
            ...
            "risk_signal_pass": bool(signal_meta["risk_signal_pass"]),
            "top_direction_id": signal_meta.get("top_direction_id"),
            "bottom_direction_id": signal_meta.get("bottom_direction_id"),
            "delta_accept_top_vs_bottom": signal_meta.get("delta_accept_top_vs_bottom"),
            "delta_flip_top_vs_bottom": signal_meta.get("delta_flip_top_vs_bottom"),
            "delta_intrusion_top_vs_bottom": signal_meta.get("delta_intrusion_top_vs_bottom"),
            "delta_margin_top_vs_bottom": signal_meta.get("delta_margin_top_vs_bottom"),
        }
        for _, corr_row in corr_df.iterrows():
            s_name = str(corr_row["score_name"])
            m_name = str(corr_row["metric_name"])
            row[f"pearson__{s_name}__{m_name}"] = corr_row["pearson_r"]
            row[f"spearman__{s_name}__{m_name}"] = corr_row["spearman_rho"]
        summary_rows.append(row)

        print(
            f"[C0][{args.dataset}][seed={seed}] "
            f"score_pass={signal_meta['score_signal_pass']} "
            f"risk_pass={signal_meta['risk_signal_pass']} "
    ...
    summary_df.to_csv(os.path.join(dataset_out, "summary_per_run.csv"), index=False)
    agg_df = summary_df.drop(columns=["seed", "setting", "split_hash"], errors="ignore").groupby(["dataset"], as_index=False).mean(numeric_only=True)
    agg_df.to_csv(os.path.join(dataset_out, "summary_agg.csv"), index=False)
