...
def _merge_apply_summaries(
    ...
    *,
    ...
    return {
        ...
        "pool_mode": str(train_result.summary.get("pool_mode", "same_only")),
        ...
        "response_stats_window_count": int(train_result.summary.get("response_stats_window_count", 0)),
        ...
        "same_proto_effective_sample_size": float(train_result.summary.get("same_proto_effective_sample_size", 0.0)),
        "opp_proto_effective_sample_size": float(train_result.summary.get("opp_proto_effective_sample_size", 0.0)),
...
def _bipolar_anchor_diag_rows(dataset: str, seed: int, arm_name: str, operator) -> List[Dict[str, object]]:
    rows = list(operator.meta.get("pair_weight_rows", []))
    if not rows:
        return []
    out: List[Dict[str, object]] = []
    for row in rows:
        out.append(
            {
                ...
                "seed": int(seed),
                "arm": str(arm_name),
                "class_id": int(row["same_class_id"]),
                "prototype_id": int(row["same_prototype_id"]),
                "opp_class_id": int(row["opp_class_id"]),
                "opp_prototype_id": int(row["opp_prototype_id"]),
                "fit_anchor_count": int(row["same_pool_count"]),
                "same_pool_count": int(row["same_pool_count"]),
                "opp_pool_count": int(row["opp_pool_count"]),
                "fit_anchor_margin_mean": np.nan,
                "fit_anchor_same_dist_mean": np.nan,
                "anchor_coverage_ratio_mean": np.nan,
                "weight_mean": np.nan,
                "weight_min": np.nan,
                "weight_max": np.nan,
                ...
                "effective_sample_ratio": np.nan,
                "weight_scale": np.nan,
                "same_weight_mass": float(row["same_weight_mass"]),
                "opp_weight_mass": float(row["opp_weight_mass"]),
                "same_opp_count_ratio": float(row["same_opp_count_ratio"]),
                "same_opp_weight_mass_ratio": float(row["same_opp_weight_mass_ratio"]),
                "same_proto_effective_sample_size": float(row["same_proto_effective_sample_size"]),
                "opp_proto_effective_sample_size": float(row["opp_proto_effective_sample_size"]),
                "same_weight_scale": float(row["same_weight_scale"]),
                "opp_weight_scale": float(row["opp_weight_scale"]),
                "same_fallback_unweighted": bool(row["same_fallback_unweighted"]),
                "opp_fallback_unweighted": bool(row["opp_fallback_unweighted"]),
                "weight_kernel_name": str(operator.meta.get("weight_kernel_name", "")),
                "fallback_unweighted": np.nan,
            }
        )
    return out
...
def main() -> None:
    p = argparse.ArgumentParser(description="SCP-Branch P0a.1 C3 refine probe: linear targets and deployment recalibration")
    p.add_argument("--datasets", type=str, default="natops")
    p.add_argument("--seeds", type=str, default="1")
    p.add_argument("--out-root", type=str, default="out/_active/verify_route_b_pia_operator_p0a1_c3_refine_20260401_smoke")
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            train_deploy_windows = np.concatenate(
                [np.asarray(v, dtype=np.float64) for v in state.train.z_seq_list],
                axis=0,
            ).astype(np.float64)
            op_c3 = fit_single_template_pia_operator_discriminative(
                ...
                cfg=SingleTemplatePIADiscriminativeConfig(
                    ...
                    target_mode="logit_soft",
                    opp_pair_rule=str(args.c3_opp_pair_rule),
                    seed=int(seed),
                ),
            )
            op_c3l = fit_single_template_pia_operator_discriminative(
                geometry=geometry,
                cfg=SingleTemplatePIADiscriminativeConfig(
                    r_dimension=1,
                    n_iters=int(args.pia_n_iters),
            ...
            op_c3lr = fit_single_template_pia_operator_discriminative(
                geometry=geometry,
                cfg=SingleTemplatePIADiscriminativeConfig(
                    r_dimension=1,
                    n_iters=int(args.pia_n_iters),
            ...
            anchor_rows_all.extend(_bipolar_anchor_diag_rows(dataset, seed, "c3_logit_fitpool_a2r", op_c3))
            anchor_rows_all.extend(_bipolar_anchor_diag_rows(dataset, seed, "c3l_linear_fitpool_a2r", op_c3l))
            anchor_rows_all.extend(_bipolar_anchor_diag_rows(dataset, seed, "c3lr_linear_deployrecal_a2r", op_c3lr))
            ...
            c3_train, c3_val, c3_test, c3_summary, c3_state, after_c3_diag = _run_a2r_arm(op_c3, "c3_logit_fitpool_a2r")
            c3l_train, c3l_val, c3l_test, c3l_summary, c3l_state, after_c3l_diag = _run_a2r_arm(op_c3l, "c3l_linear_fitpool_a2r")
            c3lr_train, c3lr_val, c3lr_test, c3lr_summary, c3lr_state, after_c3lr_diag = _run_a2r_arm(op_c3lr, "c3lr_linear_deployrecal_a2r")
            ...
            _tc3, c3_train_metrics, c3_val_metrics, c3_test_metrics, c3_meta = _evaluate_terminal(c3_state, seed=int(seed), args=args)
            _tc3l, c3l_train_metrics, c3l_val_metrics, c3l_test_metrics, c3l_meta = _evaluate_terminal(c3l_state, seed=int(seed), args=args)
            _tc3lr, c3lr_train_metrics, c3lr_val_metrics, c3lr_test_metrics, c3lr_meta = _evaluate_terminal(c3lr_state, seed=int(seed), args=args)

            _write_json(
                os.path.join(seed_dir, "pia_operator_p0a1_c3_result.json"),
                {
                    ...
                    "c3_logit_fitpool_a2r": {"test_macro_f1": float(c3_test_metrics["macro_f1"]), "summary": dict(c3_summary), "meta": dict(c3_meta)},
                    "c3l_linear_fitpool_a2r": {"test_macro_f1": float(c3l_test_metrics["macro_f1"]), "summary": dict(c3l_summary), "meta": dict(c3l_meta)},
                    "c3lr_linear_deployrecal_a2r": {"test_macro_f1": float(c3lr_test_metrics["macro_f1"]), "summary": dict(c3lr_summary), "meta": dict(c3lr_meta)},
                    ...
                    "structure_after_c3": dict(after_c3_diag),
                    "structure_after_c3l": dict(after_c3l_diag),
                    "structure_after_c3lr": dict(after_c3lr_diag),
                },
            )
            ...
            per_seed_rows.append(
                {
                    ...
                    "c3_logit_fitpool_a2r_test_macro_f1": float(c3_test_metrics["macro_f1"]),
                    "c3l_linear_fitpool_a2r_test_macro_f1": float(c3l_test_metrics["macro_f1"]),
                    "c3lr_linear_deployrecal_a2r_test_macro_f1": float(c3lr_test_metrics["macro_f1"]),
                    ...
                    "delta_c3_vs_no_shaping": float(c3_test_metrics["macro_f1"] - base_test["macro_f1"]),
                    "delta_c3l_vs_no_shaping": float(c3l_test_metrics["macro_f1"] - base_test["macro_f1"]),
                    "delta_c3lr_vs_no_shaping": float(c3lr_test_metrics["macro_f1"] - base_test["macro_f1"]),
                    "delta_c2_vs_c0": float(c2_test_metrics["macro_f1"] - c0_test_metrics["macro_f1"]),
                    "delta_c3_vs_c0": float(c3_test_metrics["macro_f1"] - c0_test_metrics["macro_f1"]),
                    "delta_c3l_vs_c0": float(c3l_test_metrics["macro_f1"] - c0_test_metrics["macro_f1"]),
                    "delta_c3lr_vs_c0": float(c3lr_test_metrics["macro_f1"] - c0_test_metrics["macro_f1"]),
                    ...
                    "c3_response_vs_margin_correlation": float(c3_summary["response_vs_margin_correlation"]),
                    "c3l_response_vs_margin_correlation": float(c3l_summary["response_vs_margin_correlation"]),
                    "c3lr_response_vs_margin_correlation": float(c3lr_summary["response_vs_margin_correlation"]),
                    ...
                    "c3_activation_coverage_ratio": float(c3_summary["activation_coverage_ratio"]),
                    "c3l_activation_coverage_ratio": float(c3l_summary["activation_coverage_ratio"]),
                    "c3lr_activation_coverage_ratio": float(c3lr_summary["activation_coverage_ratio"]),
                    ...
                    "c3_template_mean_direction_cosine": float(c3_summary["template_mean_direction_cosine"]),
                    "c3l_template_mean_direction_cosine": float(c3l_summary["template_mean_direction_cosine"]),
                    "c3lr_template_mean_direction_cosine": float(c3lr_summary["template_mean_direction_cosine"]),
                    ...
                    "c3_effective_sample_size": float(c3_summary["effective_sample_size"]),
                    "c3l_effective_sample_size": float(c3l_summary["effective_sample_size"]),
                    "c3lr_effective_sample_size": float(c3lr_summary["effective_sample_size"]),
                    ...
                    "c3_effective_sample_ratio": float(c3_summary["effective_sample_ratio"]),
                    "c3l_effective_sample_ratio": float(c3l_summary["effective_sample_ratio"]),
                    "c3lr_effective_sample_ratio": float(c3lr_summary["effective_sample_ratio"]),
                    ...
                    "c3_same_proto_effective_sample_size": float(c3_summary["same_proto_effective_sample_size"]),
                    "c3_opp_proto_effective_sample_size": float(c3_summary["opp_proto_effective_sample_size"]),
                    "c3l_same_proto_effective_sample_size": float(c3l_summary["same_proto_effective_sample_size"]),
                    "c3l_opp_proto_effective_sample_size": float(c3l_summary["opp_proto_effective_sample_size"]),
                    "c3lr_same_proto_effective_sample_size": float(c3lr_summary["same_proto_effective_sample_size"]),
                    "c3lr_opp_proto_effective_sample_size": float(c3lr_summary["opp_proto_effective_sample_size"]),
                    ...
                    "c3_same_pool_count": int(c3_summary["same_pool_count"]),
                    "c3_opp_pool_count": int(c3_summary["opp_pool_count"]),
                    "c3_same_weight_mass": float(c3_summary["same_weight_mass"]),
                    "c3_opp_weight_mass": float(c3_summary["opp_weight_mass"]),
                    "c3_same_opp_count_ratio": float(c3_summary["same_opp_count_ratio"]),
                    "c3_same_opp_weight_mass_ratio": float(c3_summary["same_opp_weight_mass_ratio"]),
                    "c3_fit_target_mode": str(c3_summary["fit_target_mode"]),
                    "c3_target_mode": str(c3_summary["target_mode"]),
                    "c3_response_stats_mode": str(c3_summary["response_stats_mode"]),
                    "c3_response_stats_window_count": int(c3_summary["response_stats_window_count"]),
                    "c3l_fit_target_mode": str(c3l_summary["fit_target_mode"]),
                    "c3l_target_mode": str(c3l_summary["target_mode"]),
                    "c3l_response_stats_mode": str(c3l_summary["response_stats_mode"]),
                    "c3l_response_stats_window_count": int(c3l_summary["response_stats_window_count"]),
                    "c3lr_fit_target_mode": str(c3lr_summary["fit_target_mode"]),
                    "c3lr_target_mode": str(c3lr_summary["target_mode"]),
                    "c3lr_response_stats_mode": str(c3lr_summary["response_stats_mode"]),
                    "c3lr_response_stats_window_count": int(c3lr_summary["response_stats_window_count"]),
                    "c3_pool_mode": str(c3_summary["pool_mode"]),
                    "c3_opp_pair_rule": str(c3_summary["opp_pair_rule"]),
                    "c3_discriminative_target_gap": float(c3_summary["discriminative_target_gap"]),
                    "c3l_discriminative_target_gap": float(c3l_summary["discriminative_target_gap"]),
                    "c3lr_discriminative_target_gap": float(c3lr_summary["discriminative_target_gap"]),
                    ...
                    "c3_proto_weight_scale_mean": float(c3_summary["proto_weight_scale_mean"]),
                    "c3l_proto_weight_scale_mean": float(c3l_summary["proto_weight_scale_mean"]),
                    "c3lr_proto_weight_scale_mean": float(c3lr_summary["proto_weight_scale_mean"]),
                    ...
                    "c3_proto_weight_scale_min": float(c3_summary["proto_weight_scale_min"]),
                    "c3l_proto_weight_scale_min": float(c3l_summary["proto_weight_scale_min"]),
                    "c3lr_proto_weight_scale_min": float(c3lr_summary["proto_weight_scale_min"]),
                    "c0_weight_kernel_name": str(c0_summary["weight_kernel_name"]),
                    "c2_weight_kernel_name": str(c2_summary["weight_kernel_name"]),
                    "c3_weight_kernel_name": str(c3_summary["weight_kernel_name"]),
                    "c3l_weight_kernel_name": str(c3l_summary["weight_kernel_name"]),
                    "c3lr_weight_kernel_name": str(c3lr_summary["weight_kernel_name"]),
                    ...
                    "delta_c3_margin": float(after_c3_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]),
                    "delta_c3l_margin": float(after_c3l_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]),
                    "delta_c3lr_margin": float(after_c3lr_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]),
                    ...
                    "c3_margin_gain_per_unit_distortion": float(
                        (after_c3_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]) / max(1e-6, float(c3_summary["operator_to_step_ratio_mean"]))
                    ),
                    "c3l_margin_gain_per_unit_distortion": float(
                        (after_c3l_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]) / max(1e-6, float(c3l_summary["operator_to_step_ratio_mean"]))
                    ),
                    "c3lr_margin_gain_per_unit_distortion": float(
                        (after_c3lr_diag["nearest_prototype_margin"] - before_diag["nearest_prototype_margin"]) / max(1e-6, float(c3lr_summary["operator_to_step_ratio_mean"]))
                    ),
                }
            )
            ...
            for stage_name, diag in [
                ...
                ("after_c3_logit_fitpool_a2r", after_c3_diag),
                ("after_c3l_linear_fitpool_a2r", after_c3l_diag),
                ("after_c3lr_linear_deployrecal_a2r", after_c3lr_diag),
            ]:
            ...
            score_specs = [
                ...
                ("c3_logit_fitpool_a2r", c3_train_metrics, c3_val_metrics, c3_test_metrics),
                ("c3l_linear_fitpool_a2r", c3l_train_metrics, c3l_val_metrics, c3l_test_metrics),
                ("c3lr_linear_deployrecal_a2r", c3lr_train_metrics, c3lr_val_metrics, c3lr_test_metrics),
            ]
            ...
            for summary in [c0_summary, c2_summary, c3_summary, c3l_summary, c3lr_summary]:
            ...
            config_rows.append(
                {
                    ...
                    "c3_target_pos": float(args.c3_target_pos),
                    "c3_target_neg": float(args.c3_target_neg),
                    "c3_opp_pair_rule": str(args.c3_opp_pair_rule),
                    "c3_target_modes_compared": "logit_soft,linear_pm1",
                    "c3_response_stats_modes_compared": "fit_pool,deployment_train",
    ...
    config_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_config_table.csv")
    per_seed_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_per_seed.csv")
    structure_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_structure_diagnostics.csv")
    score_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_score_diagnostics.csv")
    response_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_response_diagnostics.csv")
    anchor_csv = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_anchor_diagnostics.csv")
    conclusion_md = os.path.join(args.out_root, "pia_operator_p0a1_c3_refine_conclusion.md")
    ...
    for dataset in datasets:
        ...
        lines.append(f"- `c3_logit_fitpool_a2r`: {_format_mean_std(ds['c3_logit_fitpool_a2r_test_macro_f1'].tolist())}")
        lines.append(f"- `c3l_linear_fitpool_a2r`: {_format_mean_std(ds['c3l_linear_fitpool_a2r_test_macro_f1'].tolist())}")
        lines.append(f"- `c3lr_linear_deployrecal_a2r`: {_format_mean_std(ds['c3lr_linear_deployrecal_a2r_test_macro_f1'].tolist())}")
        ...
        lines.append(f"- `c3_template_mean_direction_cosine`: {_format_mean_std(ds['c3_template_mean_direction_cosine'].tolist())}")
        lines.append(f"- `c3l_template_mean_direction_cosine`: {_format_mean_std(ds['c3l_template_mean_direction_cosine'].tolist())}")
        lines.append(f"- `c3lr_template_mean_direction_cosine`: {_format_mean_std(ds['c3lr_template_mean_direction_cosine'].tolist())}")
        ...
        lines.append(f"- `c3_effective_sample_ratio`: {_format_mean_std(ds['c3_effective_sample_ratio'].tolist())}")
        lines.append(f"- `c3l_effective_sample_ratio`: {_format_mean_std(ds['c3l_effective_sample_ratio'].tolist())}")
        lines.append(f"- `c3lr_effective_sample_ratio`: {_format_mean_std(ds['c3lr_effective_sample_ratio'].tolist())}")
        lines.append(f"- `c3_response_vs_margin_correlation`: {_format_mean_std(ds['c3_response_vs_margin_correlation'].tolist())}")
        lines.append(f"- `c3l_response_vs_margin_correlation`: {_format_mean_std(ds['c3l_response_vs_margin_correlation'].tolist())}")
        lines.append(f"- `c3lr_response_vs_margin_correlation`: {_format_mean_std(ds['c3lr_response_vs_margin_correlation'].tolist())}")
        lines.append(f"- `c2_min_proto_effective_sample_size`: {_format_mean_std(ds['c2_min_proto_effective_sample_size'].tolist())}")
        lines.append(f"- `c3_same_proto_effective_sample_size`: {_format_mean_std(ds['c3_same_proto_effective_sample_size'].tolist())}")
        lines.append(f"- `c3_opp_proto_effective_sample_size`: {_format_mean_std(ds['c3_opp_proto_effective_sample_size'].tolist())}")
        lines.append(f"- `c3_same_opp_count_ratio`: {_format_mean_std(ds['c3_same_opp_count_ratio'].tolist())}")
        lines.append(f"- `c3_same_opp_weight_mass_ratio`: {_format_mean_std(ds['c3_same_opp_weight_mass_ratio'].tolist())}")
        count_ratio_mean = float(np.mean(ds["c3_same_opp_count_ratio"].to_numpy(dtype=np.float64)))
        mass_ratio_mean = float(np.mean(ds["c3_same_opp_weight_mass_ratio"].to_numpy(dtype=np.float64)))
        count_imbalance = abs(np.log(max(1e-8, count_ratio_mean))) > np.log(1.5)
        mass_imbalance = abs(np.log(max(1e-8, mass_ratio_mean))) > np.log(1.25)
        if count_imbalance or mass_imbalance:
            lines.append("- `same/opp pool audit`: 存在系统性失衡，结果需同时归因于判别目标与 pool 结构。")
        else:
            lines.append("- `same/opp pool audit`: 未见明显系统性失衡，主归因可以更集中在判别目标重写。")
        lines.append("")

    with open(conclusion_md, "w", encoding="utf-8") as f:
        f.write("\n".join(lines).rstrip() + "\n")

    print(f"[pia-operator-c3-refine] wrote {config_csv}")
    print(f"[pia-operator-c3-refine] wrote {per_seed_csv}")
    print(f"[pia-operator-c3-refine] wrote {structure_csv}")
    print(f"[pia-operator-c3-refine] wrote {score_csv}")
    print(f"[pia-operator-c3-refine] wrote {response_csv}")
    print(f"[pia-operator-c3-refine] wrote {anchor_csv}")
    print(f"[pia-operator-c3-refine] wrote {conclusion_md}")
