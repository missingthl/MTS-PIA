...
def modeig_forward(P, op, eig_mode='svd', param=None):
    ...
    P_sym = sym(P)
    flat_P = P_sym.reshape(batch_size * channels, n, n)
    S, U = th.linalg.eigh(flat_P)
    U = U.reshape(batch_size, channels, n, n)
    S = S.reshape(batch_size, channels, n)
    S_fn = op.fn(S, param)
