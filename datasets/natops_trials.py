...
def _resolve_natops_root(path: str | Path) -> Path:
    p = Path(path).expanduser().resolve()
    if (p / "NATOPS_TRAIN.ts").is_file() and (p / "NATOPS_TEST.ts").is_file():
        return p
    nested = p / "NATOPS"
    if (nested / "NATOPS_TRAIN.ts").is_file() and (nested / "NATOPS_TEST.ts").is_file():
        return nested
    raise FileNotFoundError(
        "NATOPS root not found. Expected NATOPS_TRAIN.ts and NATOPS_TEST.ts. "
        f"Got: {p}"
    )
...
def _parse_ts_line(line: str) -> Tuple[np.ndarray, float]:
...
class NATOPSTrialDataset:
    """Expose NATOPS UEA-format multivariate time-series in common trial dict format."""
    ...
    def __init__(
        ...
        *,
        ...
        self.root = _resolve_natops_root(root)
        ...
        train_x, train_y_raw = _load_ts_file(self.root / "NATOPS_TRAIN.ts")
        test_x, test_y_raw = _load_ts_file(self.root / "NATOPS_TEST.ts")

        all_labels = sorted(set(train_y_raw.tolist()) | set(test_y_raw.tolist()))
        label_map = {float(lbl): idx for idx, lbl in enumerate(all_labels)}
        self._label_map = label_map

        self._cache: Dict[str, Tuple[np.ndarray, np.ndarray]] = {
            "train": (train_x, np.asarray([label_map[float(v)] for v in train_y_raw], dtype=np.int64)),
            "test": (test_x, np.asarray([label_map[float(v)] for v in test_y_raw], dtype=np.int64)),
        }
    ...
    def __iter__(self) -> Iterator[Dict]:
        for split in self.include_splits:
            ...
            for i in range(x.shape[0]):
                yield {
                    "trial_id_str": f"natops_{split}_n{i:05d}",
                    "x_trial": x[i],
                    "label": int(y[i]),
                    "sfreq": self.sfreq,
                    "subject": -1,
