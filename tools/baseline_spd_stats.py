...
def _split_groups(trials, labels, split_by: str, split_seed: int, val_ratio: float):
    if trials is None:
...
def main():
    parser = argparse.ArgumentParser(description="Trivial SPD/TSM stats baseline.")
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        "--out",
        default="logs/baseline_spd_stats_subject.json",
    ...
    train_idx, val_idx = _split_groups(
        trials, labels, args.split_by, args.split_seed, args.val_ratio
    )
    ...
    for path in file_paths:
        ...
        features.append(_feature_vector(arr))
